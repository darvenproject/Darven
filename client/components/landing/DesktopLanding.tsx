...
export default function DesktopLanding() {
  ...
  const scrollToSlide = (slideIndex: number) => {
    containerRef.current?.scrollTo({ top: slideIndex * window.innerHeight, behavior: 'smooth' })
  }
    ...
    >
      ...
      {heroImages.length > 0 && (
        <div className="h-screen w-full snap-start snap-always relative">
            ...
            >
              <div className="absolute inset-0 bg-gray-300 dark:bg-dark-surface">
                <div className="relative h-full w-full">
                  <Image src={getImageUrl(heroImage.image_url)} alt={heroImage.title || 'Hero Image'} fill sizes="100vw" priority={index === 0} loading="eager" unoptimized className="object-cover" />
                </div>
              </div>
          ...
          {heroImages.length > 1 && (
            <>
              <button onClick={() => setCurrentHeroIndex((prev) => (prev === 0 ? heroImages.length - 1 : prev - 1))} className="absolute left-4 top-1/2 transform -translate-y-1/2 p-3 bg-black bg-opacity-50 hover:bg-opacity-70 text-white rounded-full transition-all z-30" aria-label="Previous slide">
                <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
              </button>
              <button onClick={() => setCurrentHeroIndex((prev) => (prev === heroImages.length - 1 ? 0 : prev + 1))} className="absolute right-4 top-1/2 transform -translate-y-1/2 p-3 bg-black bg-opacity-50 hover:bg-opacity-70 text-white rounded-full transition-all z-30" aria-label="Next slide">
          ...
          {currentSlideIndex === 0 && (
            <button onClick={() => scrollToSlide(1)} className="absolute bottom-12 left-1/2 transform -translate-x-1/2 text-white hover:scale-110 transition-transform duration-300 animate-bounce z-20" aria-label="Scroll down">
      ...
      {sectionImages.map((item, index) => (
        <Link key={`${item.category}-${index}`} href={item.link!} className="h-screen w-full snap-start snap-always block relative">
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 0.6 }} viewport={{ once: false, amount: 0.5 }} className="h-full w-full relative group">
            <div className="absolute inset-0 bg-gray-300 dark:bg-dark-surface">
              <Image src={getImageUrl(item.image_url)} alt={item.title} fill sizes="100vw" priority unoptimized className="object-cover group-hover:scale-105 transition-transform duration-700" />
            </div>
      ...
      {/* Slide 5: Footer — single set of Instagram + Facebook only */}
      <div className="h-screen w-full snap-start snap-always relative bg-white">
        <div className="container mx-auto px-6 lg:px-12 xl:px-24 h-full flex flex-col justify-center items-center text-black max-w-screen-2xl">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} viewport={{ once: false, amount: 0.3 }} className="text-center w-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-16 lg:gap-24 mb-16 max-w-6xl mx-auto">
              ...
              {/* Contact — Instagram + Facebook ONLY, no Mail, no Twitter, no duplicate row */}
              <div>
                ...
                <ul className="space-y-3 text-base md:text-lg">
                  ...
                  <li>
                    <a href="mailto:shopdarven@gmail.com" className="hover:text-gray-600 transition-colors">
      ...
      <div className="fixed bottom-8 right-8 flex flex-col gap-3 z-40">
        ...
        {[0, 1, 2, 3, 4].map((slideIndex) => (
          <button key={slideIndex} onClick={() => scrollToSlide(slideIndex)} className={`w-3 h-3 rounded-full transition-all duration-300 ${currentSlideIndex === slideIndex ? 'bg-white scale-125' : 'bg-white bg-opacity-50 hover:bg-opacity-75'}`} aria-label={`Go to slide ${slideIndex + 1}`} />
        ))}
