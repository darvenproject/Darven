...
export default function WaistCoatPage() {
  const [products, setProducts] = useState<Product[]>([])
  const [loading, setLoading] = useState(true)
  const [selectedCategory, setSelectedCategory] = useState<string>('All')

  const fabricCategories = ['All', 'Banarsi', 'Wash n Wear', 'Boski', 'Soft Cotton', 'Giza Moon Cotton']
  ...
  const fetchProducts = async () => {
    try {
      ...
      console.error('Error fetching waist coat products:', error)
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="border-b border-gray-200">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
          ...
          <p className="text-gray-600 text-lg max-w-2xl">
            Premium traditional waistcoats to complete your kurta pajama and shalwar kameez look.
          </p>
        </div>
      </div>
        ...
        {filteredProducts.length === 0 ? (
          <div className="text-center py-20">
            ...
            <p className="text-xl font-bold text-gray-900 mb-2">
              No waist coats available
            </p>
            <p className="text-gray-600">
              {selectedCategory === 'All'
                ? 'Check back soon for new waist coat styles'
                : `No waist coats in ${selectedCategory} category`
              }
            </p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
            {filteredProducts.map((product, index) => (
              ...
              >
                <Link href={`/waist-coat/${product.id}`}>
                  <div className="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 border-2 border-gray-200 hover:border-gray-900 hover:-translate-y-2">
                    ...
                    <div className="p-4 bg-gray-50 group-hover:bg-white transition-colors">
                      <h3 className="text-base sm:text-lg font-black text-gray-900 mb-2 line-clamp-1 group-hover:text-gray-600 transition-colors">
                        {product.name}
                      </h3>
                      ...
                      <div className="flex items-end justify-between mt-4">
                        ...
                        <div className="w-10 h-10 bg-gray-200 rounded-full flex items-center justify-center group-hover:bg-gray-900 transition-all group-hover:scale-110">
                          <svg className="w-5 h-5 text-gray-900 group-hover:text-white transition-colors group-hover:translate-x-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
