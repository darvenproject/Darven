...
const getLovedByCount = (id: number) => {
  const base = (id * 193 + 77) % 600
  return Math.max(80, base + 150)
}

export default function WaistCoatDetailPage() {
  ...
  const shalwarSizeChart = [
    { size: 'XS', length: 39 }, { size: 'S', length: 40 },
    { size: 'M', length: 42 }, { size: 'L', length: 43 }, { size: 'XL', length: 44 },
  ]

  const pajamaSizeChart = [
    { size: 'XS', length: 37, waist: 27, hips: 38 }, { size: 'S', length: 38, waist: 29, hips: 39 },
    { size: 'M', length: 39, waist: 31, hips: 40 }, { size: 'L', length: 40, waist: 33, hips: 42 },
    { size: 'XL', length: 41, waist: 34, hips: 44 },
  ]

  useEffect(() => { fetchProduct(); fetchRelatedProducts() }, [params.id])

  const fetchProduct = async () => {
    try {
      const response = await apiClient.getWaistCoatProduct(params.id as string)
  ...
  const fetchRelatedProducts = async () => {
    try {
      const response = await apiClient.getWaistCoatProducts()
  ...
  const handleAddToCart = () => {
    ...
    addItem({
      id: `waist-coat-${product.id}-${selectedSize}-${selectedColor}-${Date.now()}`,
      type: 'waist-coat',
      name: product.name,
      price: product.price,
      quantity: quantity,
  ...
  if (!product) return (
    <div className="min-h-screen flex items-center justify-center bg-white">
      <div className="text-center">
        ...
        <Link href="/waist-coat" className="text-gray-600 hover:text-gray-900 font-bold">← Back to Waist Coat</Link>
      </div>
    </div>
  )
  ...
  return (
    <div className="min-h-screen bg-white">
      <StructuredData
        data={generateProductSchema(product, 'https://shopdarven.pk', 'waist-coat', 'Apparel > Men > Waist Coat')}
      />
      {showSizeChart && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-y-auto shadow-2xl border-2 border-gray-200">
            <div className="sticky top-0 bg-white border-b border-gray-200 p-6 flex items-center justify-between">
              ...
              <button onClick={() => setShowSizeChart(false)} className="w-10 h-10 flex items-center justify-center rounded-xl hover:bg-gray-100 transition-colors"><FiX className="w-6 h-6 text-gray-600" /></button>
            </div>
            <div className="p-6 space-y-8">
              <div>
                ...
                <div className="overflow-x-auto"><table className="w-full border-2 border-gray-200 rounded-xl overflow-hidden"><thead><tr className="bg-gray-900">{['Size','Collar','Shoulder','Chest','Sleeves','Length'].map(h=><th key={h} className="px-4 py-3 text-sm font-black text-white text-center first:text-left">{h}</th>)}</tr></thead><tbody>{kameezSizeChart.map((row,i)=><tr key={row.size} className={i%2===0?'bg-gray-50':'bg-white'}><td className="px-4 py-3 text-left font-black text-gray-900 border-r border-gray-200">{row.size}</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.collar}"</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.shoulder}"</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.chest}"</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.sleeves}"</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.length}"</td></tr>)}</tbody></table></div>
              </div>
              <div>
                ...
                <div className="overflow-x-auto"><table className="w-full border-2 border-gray-200 rounded-xl overflow-hidden"><thead><tr className="bg-gray-900">{['Size','Length'].map(h=><th key={h} className="px-4 py-3 text-sm font-black text-white text-center first:text-left">{h}</th>)}</tr></thead><tbody>{shalwarSizeChart.map((row,i)=><tr key={row.size} className={i%2===0?'bg-gray-50':'bg-white'}><td className="px-4 py-3 text-left font-black text-gray-900 border-r border-gray-200">{row.size}</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.length}"</td></tr>)}</tbody></table></div>
              </div>
              <div>
                ...
                <div className="overflow-x-auto"><table className="w-full border-2 border-gray-200 rounded-xl overflow-hidden"><thead><tr className="bg-gray-900">{['Size','Length','Waist','Hips'].map(h=><th key={h} className="px-4 py-3 text-sm font-black text-white text-center first:text-left">{h}</th>)}</tr></thead><tbody>{pajamaSizeChart.map((row,i)=><tr key={row.size} className={i%2===0?'bg-gray-50':'bg-white'}><td className="px-4 py-3 text-left font-black text-gray-900 border-r border-gray-200">{row.size}</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.length}"</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.waist}"</td><td className="px-4 py-3 text-center text-gray-700 font-bold">{row.hips}"</td></tr>)}</tbody></table></div>
              </div>
              <div className="bg-gray-50 rounded-xl p-6 border-2 border-gray-200">
                ...
                <p className="text-xs text-gray-600 italic mt-4">* All measurements are in inches.</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="border-b border-gray-200">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center gap-2 text-sm">
            <Link href="/waist-coat" className="text-gray-600 hover:text-gray-900 font-bold">Waist Coat</Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 mb-16">
          <div className="space-y-4">
            <div className="relative aspect-[3/4] rounded-2xl overflow-hidden bg-gray-50 border-2 border-gray-200">
              <img src={getImageUrl(product.images[selectedImage])} alt={`${product.name} - Waist Coat`} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.src = '/placeholder.jpg' }} />
            </div>
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 bg-gray-50 rounded-xl"><div className="flex items-center gap-2 mb-2"><FiPackage className="w-5 h-5 text-gray-600" /><span className="text-sm font-bold text-gray-600">Material</span></div><span className="text-base font-black text-gray-900">{product.material}</span></div>
              <div className="p-4 bg-gray-50 rounded-xl"><div className="flex items-center gap-2 mb-2"><FiTruck className="w-5 h-5 text-gray-600" /><span className="text-sm font-bold text-gray-600">Stock</span></div><span className={`text-base font-black ${product.stock > 0 ? 'text-green-600' : 'text-red-600'}`}>{product.stock > 0 ? `${product.stock} available` : 'Out of stock'}</span></div>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setShowSizeChart(true)} className="group px-4 py-2 rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-900 hover:to-gray-800 border-2 border-gray-300 hover:border-gray-900 transition-all duration-300 hover:scale-105 hover:shadow-lg">
                  <div className="flex items-center gap-2"><div className="relative"><FiInfo className="w-4 h-4 text-gray-700 group-hover:text-white transition-colors" /><div className="absolute -top-1 -right-1 w-2 h-2 bg-red-500 rounded-full animate-pulse"></div></div><span className="text-sm font-black text-gray-700 group-hover:text-white transition-colors">Size Chart</span></div>
                </button>
              </div>
              <div className="grid grid-cols-5 gap-2">
                {availableSizes.map((size) => (<button key={size} onClick={() => setSelectedSize(size)} className={`py-3 px-4 rounded-xl border-2 font-black transition-all hover:scale-105 ${selectedSize === size ? 'border-gray-900 bg-gray-900 text-white scale-105 shadow-lg' : 'border-gray-300 bg-white text-gray-900 hover:border-gray-500 hover:shadow-md'}`}>{size}</button>))}
              </div>
            </div>
          ...
          <div>
            ...
            <div className="lg:hidden overflow-x-auto pb-4 -mx-4 px-4">
              <div className="flex gap-4 min-w-max">
                {relatedProducts.map((rp) => (<Link key={rp.id} href={`/waist-coat/${rp.id}`}><div className="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 border-2 border-gray-200 hover:border-gray-900 hover:-translate-y-2 w-[280px]"><div className="relative aspect-[3/4] overflow-hidden bg-gray-100"><img src={getImageUrl(rp.images[0])} alt={rp.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" /></div><div className="p-4 bg-gray-50 group-hover:bg-white transition-colors"><h3 className="text-base font-black text-gray-900 mb-2 line-clamp-1">{rp.name}</h3><span className="text-lg font-black text-gray-900">Rs {rp.price.toLocaleString()}</span></div></div></Link>))}
              </div>
            </div>
            <div className="hidden lg:grid grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
              {relatedProducts.map((rp) => (<Link key={rp.id} href={`/waist-coat/${rp.id}`}><div className="group bg-white rounded-2xl overflow-hidden shadow-md hover:shadow-2xl transition-all duration-500 border-2 border-gray-200 hover:border-gray-900 hover:-translate-y-2"><div className="relative aspect-[3/4] overflow-hidden bg-gray-100"><img src={getImageUrl(rp.images[0])} alt={rp.name} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-700" loading="lazy" /><div className="absolute inset-0 bg-gradient-to-t from-black/30 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300" /></div><div className="p-4 bg-gray-50 group-hover:bg-white transition-colors"><h3 className="text-base font-black text-gray-900 mb-2 line-clamp-1">{rp.name}</h3><span className="text-xl font-black text-gray-900">Rs {rp.price.toLocaleString()}</span></div></div></Link>))}
            </div>
          </div>
        )}
      </div>
