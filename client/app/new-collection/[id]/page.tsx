...
export default function NewCollectionDetailPage() {
  ...
  const fetchProduct = async () => {
    try {
      ...
      console.error('Error fetching new collection product:', error)
    } finally {
      setLoading(false)
    }
  }
  ...
  const handleAddToCart = () => {
    ...
    addItem({
      // FIX: use 'ready-made' as the type since cartStore only accepts
      // 'custom' | 'ready-made' | 'fabric'
      // If you want a distinct type, add 'new-collection' to cartStore.ts (see note below)
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white">
        <div className="text-center">
          ...
          <Link href="/new-collection" className="text-gray-600 hover:text-gray-900 font-bold">
    ...
    <div className="min-h-screen bg-white">
            ...
            <div className="p-6 space-y-8">
              ...
              <div>
                <h3 className="text-xl font-black text-gray-900 mb-4 flex items-center gap-2">
                  <FiPackage className="w-5 h-5" /> Kameez Measurements
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full border-2 border-gray-200 rounded-xl overflow-hidden">
                    <thead>
                      <tr className="bg-gray-900">
                        {['Size','Collar','Shoulder','Chest','Sleeves','Length'].map(h => (
                          <th key={h} className="px-4 py-3 text-sm font-black text-white text-center first:text-left">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {kameezSizeChart.map((row, index) => (
                        <tr key={row.size} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                          <td className="px-4 py-3 text-left font-black text-gray-900 border-r border-gray-200">{row.size}</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.collar}"</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.shoulder}"</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.chest}"</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.sleeves}"</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.length}"</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div>
                <h3 className="text-xl font-black text-gray-900 mb-4 flex items-center gap-2">
                  <FiPackage className="w-5 h-5" /> Shalwar Measurements
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full border-2 border-gray-200 rounded-xl overflow-hidden">
                    <thead>
              ...
              <div>
                <h3 className="text-xl font-black text-gray-900 mb-4 flex items-center gap-2">
                  <FiPackage className="w-5 h-5" /> Pajama Measurements
                </h3>
                <div className="overflow-x-auto">
                  <table className="w-full border-2 border-gray-200 rounded-xl overflow-hidden">
                    <thead>
                      <tr className="bg-gray-900">
                        {['Size','Length','Waist','Hips'].map(h => (
                          <th key={h} className="px-4 py-3 text-sm font-black text-white text-center first:text-left">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {pajamaSizeChart.map((row, index) => (
                        <tr key={row.size} className={index % 2 === 0 ? 'bg-gray-50' : 'bg-white'}>
                          <td className="px-4 py-3 text-left font-black text-gray-900 border-r border-gray-200">{row.size}</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.length}"</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.waist}"</td>
                          <td className="px-4 py-3 text-center text-gray-700 font-bold">{row.hips}"</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="bg-gray-50 rounded-xl p-6 border-2 border-gray-200">
                <h4 className="text-lg font-black text-gray-900 mb-3 flex items-center gap-2">
                  <FiInfo className="w-5 h-5" /> How to Measure
                </h4>
                  ...
                  <div>
                    ...
                    <ul className="space-y-2 text-sm text-gray-700">
                      {[
                        ...
                        ['Sleeves', 'Measure from shoulder to wrist'],
                        ['Length', 'Measure from shoulder to desired length'],
                      ].map(([label, desc]) => (
                        <li key={label} className="flex items-start gap-2">
                          <span className="font-black text-gray-900 mt-0.5">•</span>
                          <span><strong className="font-black">{label}:</strong> {desc}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-16 mb-16">
          ...
          <div className="space-y-6">
            ...
            <div className="py-6 border-y border-gray-200">
              <span className="text-3xl sm:text-5xl font-black text-gray-900">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-gray-50 rounded-xl">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-sm font-bold text-gray-600">Stock</span>
                </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                ...
                <button
                  ...
                  className="group px-4 py-2 rounded-xl bg-gradient-to-r from-gray-100 to-gray-200 hover:from-gray-900 hover:to-gray-800 border-2 border-gray-300 hover:border-gray-900 transition-all duration-300 hover:scale-105 hover:shadow-lg"
                >
                  <div className="flex items-center gap-2">
                    <div className="relative">
                      <FiInfo className="w-4 h-4 text-gray-700 group-hover:text-white transition-colors" />
                  ...
                  <button
                    ...
                    className={`py-3 px-4 rounded-xl border-2 font-black transition-all hover:scale-105 ${
                      selectedSize === size
                        ...
                        : 'border-gray-300 bg-white text-gray-900 hover:border-gray-500 hover:shadow-md'
                    }`}
                  >
                    {size}
                  </button>
            ...
            <div>
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-lg hover:bg-gray-900 hover:text-white transition-all hover:scale-110 border-2 border-gray-200 hover:border-gray-900"
                >
                ...
                <button
                  ...
                  className="w-10 h-10 flex items-center justify-center bg-gray-100 rounded-lg hover:bg-gray-900 hover:text-white transition-all hover:scale-110 border-2 border-gray-200 hover:border-gray-900 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:bg-gray-100"
                >
            ...
            <button
              ...
              className="w-full py-3 md:py-5 px-4 md:px-8 rounded-xl font-black text-base md:text-lg bg-gray-900 text-white hover:bg-black hover:shadow-2xl hover:scale-105 disabled:bg-gray-300 disabled:text-gray-500 disabled:cursor-not-allowed disabled:hover:scale-100 disabled:hover:shadow-none transition-all active:scale-95 flex items-center justify-center gap-2 md:gap-3"
            >
        ...
        {relatedProducts.length > 0 && (
          <div>
            <h2 className="text-3xl sm:text-4xl font-black text-gray-900 mb-8 tracking-tight">
              You Might Also Like
            </h2>
            ...
            <div className="lg:hidden overflow-x-auto pb-4 -mx-4 px-4">
              <div className="flex gap-4 min-w-max">
                {relatedProducts.map((rp) => (
                  <Link key={rp.id} href={`/new-collection/${rp.id}`}>
